...
SELECT * FROM (
        SELECT
            ...
            78085 AS AMOUNT,
            'KAKAOPAY' AS PAY_METHOD,
            1 AS BUYER_IDX,
            4 AS SELLER_IDX,
            1 AS PRODUCT_IDX,
            'MEET' AS TRADE_TYPE,
            '전라남도 논산시 반포대169가 (옥순양윤동)' AS DELIVERY_ADDRESS
        UNION ALL
    SELECT 'adbd95cc-26c0-4abd-9f0c-c7e00e026d7e', 128653, 'KAKAOPAY', 5, 6, 2, 'DELIVERY', '전라북도 수원시 팔달구 선릉거리' UNION ALL
    SELECT 'dd6bed9d-ba45-4d3b-a844-606e3cfdfe2b', 79594, 'KAKAOPAY', 4, 4, 3, 'MEET', '경상남도 보령시 가락408로' UNION ALL
    SELECT 'f5591955-ce0b-4584-ac08-7488db9c6b31', 145302, 'BANK', 5, 3, 4, 'DELIVERY', '제주특별자치도 안양시 동안구 봉은사3로 (현정김마을)' UNION ALL
    SELECT '55203d3b-76c7-49bb-b54e-be54a93c7de9', 25570, 'KAKAOPAY', 2, 6, 5, 'DELIVERY', '대전광역시 종로구 서초대1로 (영수이김동)'
) AS tmp
...
SELECT * FROM (
    SELECT 'a57d6b00-1304-48d3-a5c8-e47824fcb26b', 'CANCELLED', '2025-05-20' UNION ALL
    SELECT 'adbd95cc-26c0-4abd-9f0c-c7e00e026d7e', 'CANCELLED', '2025-05-20' UNION ALL
    SELECT 'dd6bed9d-ba45-4d3b-a844-606e3cfdfe2b', 'CANCELLED', '2025-05-20' UNION ALL
    SELECT 'f5591955-ce0b-4584-ac08-7488db9c6b31', 'PAID', '2025-05-20' UNION ALL
    SELECT '55203d3b-76c7-49bb-b54e-be54a93c7de9', 'PAID', '2025-05-20'
) AS tmp
