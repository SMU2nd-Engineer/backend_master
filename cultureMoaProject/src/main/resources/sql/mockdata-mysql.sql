...
SELECT * FROM (
    ...
    UNION ALL
    SELECT 'adbd95cc-26c0-4abd-9f0c-c7e00e026d7e', 128653, 1, 5, 6, 2, '전라북도 수원시 팔달구 선릉거리' UNION ALL
    SELECT 'dd6bed9d-ba45-4d3b-a844-606e3cfdfe2b', 79594, 1, 4, 4, 3, '경상남도 보령시 가락408로' UNION ALL
    SELECT 'f5591955-ce0b-4584-ac08-7488db9c6b31', 145302, 2, 5, 3, 4, '제주특별자치도 안양시 동안구 봉은사3로 (현정김마을)' UNION ALL
    SELECT '55203d3b-76c7-49bb-b54e-be54a93c7de9', 25570, 1, 2, 6, 5, '대전광역시 종로구 서초대1로 (영수이김동)'
) AS tmp
WHERE NOT EXISTS (SELECT 1 FROM PAYMENT_HISTORY_TBL);

-- 13.
INSERT INTO PAYMENT_STATUS_TBL (TID, CANCELED_AT, APPROVED_AT)
SELECT * FROM (
    SELECT 'a57d6b00-1304-48d3-a5c8-e47824fcb26b', '2025-05-20', NULL UNION ALL
    SELECT 'adbd95cc-26c0-4abd-9f0c-c7e00e026d7e', '2025-05-20', NULL UNION ALL
    SELECT 'dd6bed9d-ba45-4d3b-a844-606e3cfdfe2b', '2025-05-20', NULL UNION ALL
    SELECT 'f5591955-ce0b-4584-ac08-7488db9c6b31', NULL, '2025-05-20' UNION ALL
    SELECT '55203d3b-76c7-49bb-b54e-be54a93c7de9', NULL, '2025-05-20'
) AS tmp
