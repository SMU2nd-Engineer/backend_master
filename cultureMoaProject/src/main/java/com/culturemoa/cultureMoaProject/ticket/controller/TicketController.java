...
public class TicketController {
    ...
    // 검색
    @GetMapping("/search")
    public List<TicketDTO> getTicketBySearchQuery(
            @RequestParam(value = "categories", required = false) String categories,
            @RequestParam(value = "query", required = false) String query,
            @RequestParam(value = "startDate", required = false) LocalDate startDate,
            @RequestParam(value = "endDate", required = false) LocalDate endDate){
        // categories나 query가 없으면 빈 문자열 또는 null일 수 있음
        return ticketService.getTicketBySearchQuery(categories, query, startDate, endDate);
    }

    @GetMapping("/calendar")
    public List<DateCountDTO> getDateCountsByMonthAndCategories(@RequestParam("month") String month, @RequestParam(value = "categories", required = false) String categories) {
        return ticketService.getDateCountsByMonthAndCategories(month, categories);
    }
